...
for i in range(len(positive)): 
    dict_positive[positive_data['word'][i].strip()] = int(positive_data['polarity'][i])
...
for i in range(len(negative)):  
    dict_negative[negative_data['word'][i].strip()] = int(negative_data['polarity'][i])
...
def pred1():
    if request.method=='POST':
        ...
        sentiment = sentiment_analysis_dict_arabic(text)
        processed_text = get_tweet_sentiment(text)
        return render_template('result1.html', msg=processed_text, result=sentiment)
